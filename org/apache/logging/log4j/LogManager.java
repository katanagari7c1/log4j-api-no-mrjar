...
/**
 * The anchor point for the Log4j logging system. The most common usage of this class is to obtain a
 * named {@link Logger}. The method {@link #getLogger()} is provided as the most convenient way to
 * obtain a named Logger based on the calling class name. This class also provides method for
 * obtaining named Loggers that use {@link String#format(String, Object...)} style messages instead
 * of the default type of parameterized messages. These are obtained through the
 * {@link #getFormatterLogger(Class)} family of methods. Other service provider methods are given
 * through the {@link #getContext()} and {@link #getFactory()} family of methods; these methods are
 * not normally useful for typical usage of Log4j.
 */
public class LogManager {
  ...
  /**
   * Scans the classpath to find all logging implementation. Currently, only one will be used but
   * this could be extended to allow multiple implementations to be used.
   */
  static {
  ...
  /**
   * Detects if a Logger with the specified name exists. This is a convenience method for porting
   * from version 1.
   *
   * @param name
   ...
   * <p>
   * WARNING - The LoggerContext returned by this method may not be the LoggerContext used to create
   * a Logger for the calling class.
   * </p>
   *
   ...
   *          example, in a web application if the caller is a class in WEB-INF/lib then one
   *          LoggerContext may be returned and if the caller is a class in the container's
   *          classpath then a different LoggerContext may be returned. If true then only a single
  ...
  public static LoggerContext getContext(final boolean currentContext) {
    ...
    try {
      ...
      LOGGER.warn(ex.getMessage() + " Using SimpleLogger");
      return new SimpleLoggerContextFactory().getContext(FQCN, null, null, currentContext, null,
          null);
    }
  }
   ...
   *
   * @param loader
   *          The ClassLoader for the context. If null the context will attempt to determine the
   ...
   *          An external context (such as a ServletContext) to be associated with the
   *          LoggerContext.
   * @return a LoggerContext.
   */
  public static LoggerContext getContext(final ClassLoader loader, final boolean currentContext,
    ...
    try {
      ...
      LOGGER.warn(ex.getMessage() + " Using SimpleLogger");
      return new SimpleLoggerContextFactory().getContext(FQCN, loader, externalContext,
          currentContext);
    }
  }
  ...
  /**
   * Shutdown using the LoggerContext appropriate for the caller of this method. This is equivalent
   * to calling {@code LogManager.shutdown(false)}.
   *
   * This call is synchronous and will block until shut down is complete. This may include flushing
   * pending log events over network connections.
   *
   * @since 2.6
   */
  ...
  /**
   * Shutdown the logging system if the logging system supports it. This is equivalent to calling
   * {@code LogManager.shutdown(LogManager.getContext(currentContext))}.
   *
   * This call is synchronous and will block until shut down is complete. This may include flushing
   * pending log events over network connections.
   *
   * @param currentContext
   *          if true a default LoggerContext (may not be the LoggerContext used to create a Logger
   *          for the calling class) will be used. If false the LoggerContext appropriate for the
   *          caller of this method is used. For example, in a web application if the caller is a
   *          class in WEB-INF/lib then one LoggerContext may be used and if the caller is a class
   *          in the container's classpath then a different LoggerContext may be used.
   * @since 2.6
   */
  ...
  /**
   * Sets the current LoggerContextFactory to use. Normally, the appropriate LoggerContextFactory is
   * created at startup, but in certain environments, a LoggerContextFactory implementation may not
   * be available at this point. Thus, an alternative LoggerContextFactory can be set at runtime.
   *
   * <p>
   * Note that any Logger or LoggerContext objects already created will still be valid, but they
   * will no longer be accessible through LogManager. Thus, <strong>it is a bad idea to use this
   * method without a good reason</strong>! Generally, this method should be used only during
  ...
  /**
   * Returns a formatter Logger using the fully qualified name of the calling Class as the Logger
   * name.
   * <p>
   * This logger lets you use a {@link java.util.Formatter} string in the message to format
   * parameters.
   * </p>
   *
   ...
   * <p>
   * This logger let you use a {@link java.util.Formatter} string in the message to format
   * parameters.
   * </p>
   * <p>
   ...
   * @throws UnsupportedOperationException
   *           if {@code clazz} is {@code null} and the calling class cannot be determined.
  ...
  /**
   * Returns a formatter Logger using the fully qualified name of the value's Class as the Logger
   * name.
   * <p>
   * This logger let you use a {@link java.util.Formatter} string in the message to format
   * parameters.
   ...
   * @return The Logger, created with a {@link StringFormatterMessageFactory}
   * @throws UnsupportedOperationException
   *           if {@code value} is {@code null} and the calling class cannot be determined.
   * @see Logger#fatal(Marker, String, Object...)
   * @see Logger#fatal(String, Object...)
   * @see Logger#error(Marker, String, Object...)
   * @see Logger#error(String, Object...)
   ...
   *
   * @param name
   *          The logger name. If null it will default to the name of the calling class.
   * @return The Logger, created with a {@link StringFormatterMessageFactory}
   * @throws UnsupportedOperationException
   *           if {@code name} is {@code null} and the calling class cannot be determined.
   * @see Logger#fatal(Marker, String, Object...)
   * @see Logger#fatal(String, Object...)
   * @see Logger#error(Marker, String, Object...)
   * @see Logger#error(String, Object...)
   ...
   *
   * @param clazz
   *          The Class whose name should be used as the Logger name. If null it will default to the
   *          calling class.
   * @return The Logger.
   * @throws UnsupportedOperationException
   *           if {@code clazz} is {@code null} and the calling class cannot be determined.
   ...
   *          The message factory is used only when creating a logger, subsequent use does not
   *          change the logger but will log a warning if mismatched.
   * @return The Logger.
   * @throws UnsupportedOperationException
   *           if {@code clazz} is {@code null} and the calling class cannot be determined.
   */
   ...
   *
   * @param value
   *          The value whose class name should be used as the Logger name. If null the name of the
   *          calling class will be used as the logger name.
   * @return The Logger.
   * @throws UnsupportedOperationException
   *           if {@code value} is {@code null} and the calling class cannot be determined.
   */
  ...
  public static Logger getLogger(final String name) {
    return name != null ? getContext(false).getLogger(name) : getLogger(StackLocatorUtil
        .getCallerClass(2));
  }
